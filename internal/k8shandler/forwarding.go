...
func (clusterRequest *ClusterLoggingRequest) generateCollectorConfig() (config string, err error) {
	...
	generatedConfig, err := g.GenerateConf(clusterRequest.Cluster.Spec.Collection, clusterRequest.OutputSecrets,
		&clusterRequest.Forwarder.Spec, clusterRequest.Forwarder.Namespace, clusterRequest.Forwarder.Name, op)
